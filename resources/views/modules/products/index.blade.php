            ...
            <div class="col-12 col-md-4 col-lg-3 mb-md-5">
                <div class="filter-b ">
                    <div class="filter-item px-3 py-3">
                        ...
                        <form id="change-prise" class="d-flex align-items-center justify-content-between" action="{{route('productCategory',['slug'=>$slug])}}" method="get">
                            ...
                            <input name="specificationInForm" value="{{json_encode($specificationsSearch)}}" hidden>
                    ...
                    @foreach($specifications as $specification)
                        <div class="filter-item  px-3 py-3">
                            ...
                            <select class="custom-select search-redirect" id="inputGroupSelect01">
                                <option data-url="{{$valuesSearcUrl['sp_'.$specification->id]}}" value="{{'sp_'.$specification->id}}">@lang('global.no-choice')</option>
                                @foreach($specification->values as $val)
                                    @if (in_array($val->id,$valueIds))
                                        <option @if (isset($specificationsSearch[$specification->id])) @if ($specificationsSearch[$specification->id]==$val->id) selected @endif @endif data-url="{{$valuesSearcUrl[$val->id]}}" value="{{$val->id}}"> {{parseMultiLanguageString($val->value ?? null, LaravelLocalization :: getCurrentLocale ()) }}</option>
                                    @endif
                                        @endforeach
                            </select>
                        </div>
                    @endforeach
                    @if (($specifications->isNotEmpty()) &&(!empty($specificationsSearch)))
                        <div class=" px-3 py-3 d-flex align-items-center justify-content-center">
                            <a href="{{$clearUrl}}" class="reset-filters">@lang('button.update-filter')</a>
                        </div>
                    @endif
                </div>
            ...
            @if ($products->isNotEmpty())
                <div class="col-12 col-md-8 col-lg-9">
                    <div class="list-product-b mt-5 ">
                        <div class="list-product-h">{{parseMultiLanguageString($category->name ?? null, LaravelLocalization :: getCurrentLocale ()) }}</div>
                        ...
                        <div class="tab-content " id="myTabContent">
                            <div class="tab-pane fade active show" id="home1" role="tabpanel" aria-labelledby="home-tab">
                                <div class="product-list-b d-flex flex-wrap">
                                    @foreach($products as $product)
                                        <div class="product-list-item d-flex flex-column">
                                            <a href="{{route('product',['category'=>$product->category->slug,'slug'=>$product->slug])}}" class="image">
                                            ...
                                            <div class="description mt-auto">
                                                ...
                                                <div class="d-flex align-items-center mt-2 mb-3">
                                                    ...
                                                    <a data-id="{{$product->id}}" data-type="page" class="favorite-add-product checkout-link @if(in_array($product->id,$favorites)) active  @endif  ml-auto mr-2" style="cursor: pointer">
                                ...
                                <div class="debbugger" >
                                    <div> {{$products->appends(['specification' => $specificationsSearch, 'type' => $type,'to'=>$to,'from'=>$from,'search'=>$search])->render()}}</div>
                                </div>
                            </div>
